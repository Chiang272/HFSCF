...
def transform_1e_integrals_so_2(mo,H1_ao):
    ...
    int_so = np.einsum('ab,ai,bj->ij',H1_ao,so,so)
    int_so[::2,1::2] = int_so[1::2,::2] = 0

    return int_so

def transform_2e_integrals_so_2(mo,v2e_ao):

    so = np.zeros((mo.shape[0], 2 * mo.shape[1]))
    so[:, ::2] = mo
    so[:,1::2] = mo

    v2e_so = np.einsum('abcd,ai,bj,ck,dl->ijkl',v2e_ao,so,so,so,so) #this code's cost is high
    
    for i in range(len(v2e_so)):
        for j in range(len(v2e_so)):
            v2e_so[::2,1::2,i,j]= v2e_so[1::2,::2,i,j] = 0
            #v2e_so[1::2,::2,i,j] = 0
            v2e_so[i,j,::2,1::2] = v2e_so[i,j,1::2,::2] = 0
            #v2e_so[i,j,1::2,::2] = 0
...
def transform_asym_integrals_so_df(interface, mo_1, mo_2):
    ...
    v2e =  np.ascontiguousarray(einsum('uvL,up,vq->pqL',interface.v2e_ao_df, so_1, so_2, optimize = einsum_type))

    v2e = zero_spin_cases_df(v2e)
...
def zero_spin_cases_df(v2e):

    v2e[::2,1::2] = v2e[1::2,::2] = 0.0
